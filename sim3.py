...
def simulate_network(num_neurons, perturbation_set, epsilon, num_steps, transition_point):
    ...
    # Apply perturbation at time step t0
    activities_perturbed = apply_perturbation(np.copy(activities), perturbation_set, epsilon)
...
perturbation_set = [0, 1, 2, 3, 4]  # Indices of neurons to perturb
epsilon = 0.1  # Magnitude of perturbation
num_steps = 10000  # Number of time steps to run the simulation
transition_point = 0.2  # Controls the network's proximity to the chaotic transition point
x = np.arange(0, num_neurons, 1)

# Run the simulation
distances,activities,perturbed_activities = simulate_network(num_neurons, perturbation_set, epsilon, num_steps, transition_point)
